...
import { processPendingWork, queueStaleIntervalsSyncs } from "../../../lib/pipeline";
...
export async function POST(request: NextRequest) {
  ...
  const queued = await queueStaleIntervalsSyncs(10);
  return Response.json({ ok: true, queued, ...(await processPendingWork()) });
}
