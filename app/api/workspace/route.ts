...
export async function GET(request: NextRequest) {
  try {
    ...
    const stale = connection && (!connection.last_sync_at || Date.now() - new Date(String(connection.last_sync_at)).getTime() > 10 * 60 * 1000);
...
async function inviteAthlete(user: { id: string; role: string }, body: Record<string, unknown>, origin: string) {
  ...
  await db.batch([
    ...
    db.prepare("INSERT INTO notifications (id, athlete_id, recipient_email, notification_type, subject, body, status, created_at) VALUES (?, ?, ?, 'athlete_invitation', ?, ?, 'queued', ?)").bind(id("notification"), athleteId, email, "Your Personal Besties training dashboard", `${displayName}, your coach has prepared a private Personal Besties workspace for you. Visit ${origin}, enter this email address, and use the 8-digit sign-in code from the newest email.`, timestamp),
  ]);
...
async function queuePipeline(userId: string, athleteId: string) {
  const timestamp = nowIso(); const tenMinuteBucket = Math.floor(new Date(timestamp).getTime() / (10 * 60 * 1000)); const key = `intervals_sync:${athleteId}:${tenMinuteBucket}`;
