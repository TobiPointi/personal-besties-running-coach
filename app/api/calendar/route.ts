...
export async function GET(request: NextRequest) {
  try {
    ...
    const lines = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Athelon//Private Training Plan//EN", "CALSCALE:GREGORIAN", `X-WR-CALNAME:${escapeIcs(`${athlete.display_name} training plan`)}`];
    for (const session of result.results ?? []) {
      lines.push("BEGIN:VEVENT", `UID:${session.id}@athelon`, `DTSTART;VALUE=DATE:${String(session.session_date).replaceAll("-", "")}`, `SUMMARY:${escapeIcs(`${session.title}${session.planned_distance_km ? ` — ${session.planned_distance_km} km` : ""}`)}`, `DESCRIPTION:${escapeIcs([session.details, `Pace: ${session.pace_guidance ?? ""}`, `Purpose: ${session.purpose ?? ""}`, `If fatigued: ${session.fatigue_modification ?? ""}`].join("\n"))}`, "STATUS:CONFIRMED", "END:VEVENT");
    }
    lines.push("END:VCALENDAR");
    return new Response(`${lines.join("\r\n")}\r\n`, { headers: { "content-type": "text/calendar; charset=utf-8", "content-disposition": `attachment; filename="athelon-plan-v${plan.version}.ics"`, "cache-control": "private, no-store" } });
