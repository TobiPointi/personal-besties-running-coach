  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="coach-profile"><span className="avatar dark">{initials(state.user.displayName)}</span><span><strong>{state.user.displayName.split(" ")[0]}</strong><small>{coach ? "Coach workspace" : "Athlete portal"}</small></span><a href="/signout-with-chatgpt?return_to=/" aria-label="Sign out">↗</a></div>
      </aside>
...
function PlanTab({ state, coach, draft, busy, act }: { state: WorkspaceState; coach: boolean; draft?: AnyRow; busy: string; act: (action: string, payload?: AnyRow, success?: string) => void }) {
  ...
  return <section className="workspace-section"><div className="section-hero"><div><p className="eyebrow">TRAINING PLAN</p><h2>{selectedPlan ? `Version ${selectedPlan.version} · ${selectedPlan.status}` : "No plan yet"}</h2><p>{selectedPlan?.rationale ?? "Create a goal first, then generate a coach-reviewed draft."}</p></div>{coach && <div className="hero-actions"><button className="secondary-button" disabled={Boolean(busy)} onClick={() => act("generatePlan", {}, "A new draft plan was generated")}>{busy === "generatePlan" ? "Generating…" : "+ Generate draft"}</button>{draft && <button className="primary-button" disabled={Boolean(busy)} onClick={() => act("publishPlan", { planId: draft.id }, "Plan published and delivery queued")}>Publish v{draft.version}</button>}</div>}</div>
    ...
    <section className="panel"><SessionList sessions={sessions} expanded /></section>
  </section>;
}
...
function SessionList({ sessions, expanded = false }: { sessions: AnyRow[]; expanded?: boolean }) { return <div className="session-list">{sessions.length ? sessions.map((session) => <article className={`session ${expanded ? "expanded" : ""}`} key={session.id}><div className="session-date"><b>{weekday(session.session_date)}</b><span>{shortDate(session.session_date)}</span></div><div className={`session-icon ${session.major_stimulus ? "key" : ""}`}>{session.major_stimulus ? "↗" : "·"}</div><div className="session-detail"><strong>{session.title}</strong><span>{expanded ? session.details : session.pace_guidance || session.details}</span>{expanded && <small>{session.purpose}</small>}</div><b className="session-distance">{Number(session.planned_distance_km) ? `${session.planned_distance_km} km` : "Rest"}</b></article>) : <div className="empty-inline">No sessions available.</div>}</div>; }
