...
type Modal = "invite" | "goal" | "test" | "testRequest" | "performance" | "feedback" | "profile" | null;
...
function PerformanceTab({ state, coach, onAdd }: { state: WorkspaceState; coach: boolean; onAdd: () => void }) {
  ...
  const assessmentPoints = state.assessments.map((item) => ({ date:String(item.assessed_at).slice(0,10), value:scalePrediction((Number(item.race_forecast_low_seconds)+Number(item.race_forecast_high_seconds))/2, Number(state.goals[0]?.distance_km || 21.0975), distanceKm(distance)), source:"Athelon model" })).filter((item) => Number.isFinite(item.value) && item.value > 0);
  const predictionPoints = filterRange([...assessmentPoints, ...forecastPoints].sort((a,b) => a.date.localeCompare(b.date)), range);
  ...
  return <section className="workspace-section"><div className="section-hero"><div><p className="eyebrow">PERFORMANCE TRENDS</p><h2>Predictions & VO₂max</h2><p>Follow changes over your complete history—not only the last six months. Synced device estimates retain their original source.</p></div>{coach && <button className="primary-button" onClick={onAdd}>+ Add data point</button>}</div>
    ...
    <section className="panel performance-panel"><div className="performance-summary"><div><p className="eyebrow">CURRENT PREDICTION</p><strong>{latest ? formatDuration(latest.value) : "Building evidence"}</strong><small>{latest ? `${formatDate(latest.date)} · ${latest.source}` : "Add a device estimate or sync more training data"}</small></div><RangePicker value={range} setValue={setRange} /></div><TrendChart points={predictionPoints} formatter={formatDuration} inverted label={`${distanceLabel(distance)} predicted time`} /><p className="chart-footnote">Predictions are coaching estimates, not guarantees. Weather, terrain, fatigue, and race execution still matter.</p></section>
    <section className="panel performance-panel vo2-panel"><div className="performance-summary"><div><p className="eyebrow">AEROBIC FITNESS</p><h3>VO₂max history</h3><small>Garmin, Suunto, COROS, Intervals.icu, or manual lab values</small></div><RangePicker value={range} setValue={setRange} /></div><TrendChart points={vo2Points} formatter={(value)=>value.toFixed(1)} label="VO2max" /><p className="chart-footnote">Different devices use different algorithms. Compare the trend and source together rather than treating one estimate as a lab measurement.</p></section>
  </section>;
}
...
function distanceKm(distance:string) { return distance === "5k" ? 5 : distance === "10k" ? 10 : distance === "marathon" ? 42.195 : 21.0975; }
